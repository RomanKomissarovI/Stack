...
int StackPush(Stack* stk, stack_t a)
{
    ...
    *(stack_t*) (stk->data + sizeof(stack_t) * (stk->size - 1)) = a;
...
int StackCtor(Stack* stk ON_DEBUG(, const char* name, const char* file, int line))
{
    ...
    stk->data = (char*) calloc(ON_DEBUG(2 * sizeof(canar_t)) + stk->capacity * sizeof(stack_t), sizeof(char)); 

    ON_DEBUG(*(canar_t*) stk->data = k_Canar;)
    ON_DEBUG(stk->data += sizeof(canar_t);)
    ON_DEBUG(*((canar_t*) stk->data + stk->capacity * sizeof(stack_t)) = k_Canar;)
...
int Recalloc(Stack* stk, int old_size, int new_capacity)
{
    ...
    ON_DEBUG(stk->data -= sizeof(canar_t);)
    stk->data = (char*) realloc(stk->data, ON_DEBUG(2 * sizeof(canar_t) + ) new_capacity * sizeof(stack_t));
    ...
    ON_DEBUG(*(canar_t*) (stk->data + new_capacity * sizeof(stack_t)) = k_Canar);
